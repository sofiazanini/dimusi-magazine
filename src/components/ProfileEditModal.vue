<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-xs" @click.self="$emit('chiudi')">
    <div class="bg-white border border-black/10 rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-5 text-left shadow-2xl relative max-h-[90vh] overflow-y-auto">
      ...
      <!-- chiudi -->
      <button type="button" @click="$emit('chiudi')" class="absolute top-4 right-4 font-pixel text-xs text-zinc-400 hover:text-zinc-950 uppercase cursor-pointer">
        [ CHIUDI ✕ ]
      </button>
      ...
      <form @submit.prevent="inviaDati" class="space-y-4">
        ...
        <!-- img profilo -->
        <div class="space-y-1">
          <label class="font-pixel text-xs text-zinc-500 uppercase">URL FOTO PROFILO</label>
          <input v-model="moduloProfilo.avatar" type="url" placeholder="https://images.unsplash.com/..." 
            class="w-full px-3.5 py-2.5 bg-zinc-50 border border-black/10 rounded-xl font-clash text-sm focus:outline-hidden focus:border-red-mag"
          />
        </div>

        <!-- anteprima img -->
        <div v-if="moduloProfilo.avatar" class="flex items-center gap-3 p-2.5 bg-zinc-50 rounded-2xl border border-black/5">
          <img :src="moduloProfilo.avatar" alt="Anteprima" class="w-12 h-12 rounded-xl object-cover border border-black/10" />
          <span class="font-pixel text-xs text-zinc-500">Anteprima nuova immagine</span>
        </div>
